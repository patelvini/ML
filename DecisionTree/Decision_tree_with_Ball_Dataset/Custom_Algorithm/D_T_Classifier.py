...
def InfoGain(data, split_attribute_name, target_name = 'Label'):
    ...
    3. target_name = the name of the target feature. The default for this example is "Label"
    """    
...
def ID3(data, originaldata, features, target_attribute_name = 'Label', parent_node_Label = None):
    ...
    5. parent_node_Label = This is the value or Label of the mode target feature value of the parent node for a specific node. This is 
    ...
    # the mode target feature value is stored in the parent_node_Label variable.
	...
	# If none of above holds true, grow the tree
		...
		parent_node_Label = np.unique(data[target_attribute_name])[np.argmax(np.unique(data[target_attribute_name], return_counts = True)[1])]
		...
		for value in np.unique(data[best_feature]):
			...
			subtree = ID3(sub_data,data,features,target_attribute_name,parent_node_Label)
...
def train_test_split(dataset):
	...
	training_data = dataset.iloc[:85].reset_index(drop = True)
...
if __name__ == '__main__':
	...
	data = pd.read_csv('Ball_Dataset.csv', names = ['Weight', 'Surface', 'Label'])
	features = data[["Weight","Surface"]]
	target = data["Label"]
